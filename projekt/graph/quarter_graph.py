...
try:
    ...
    WHERE SUBSTR(YearQuarter, 1, 4) = :year
    ...
    if df.empty or df2.empty:
        ...
        df['Quarter'] = df['YEARQUARTER'].str[-1]
        axs[0].bar(df['Quarter'], df['TOTALTICKETSSOLD'], color='skyblue')
        axs[0].set_xlabel('Quarter')
        axs[0].set_ylabel('Total Tickets Sold')
        axs[0].set_title(f'Quarterly Ticket Sales for {year}')
        axs[0].set_xticks(df['Quarter'])
        axs[0].set_xticklabels(list_of_quarters)
        axs[0].tick_params(axis='x', rotation=45)

        df2['Quarter'] = df2['YEARQUARTER'].str[-1]
        axs[1].plot(df2['Quarter'], df2['TOTALREVENUE'], color='skyblue')
        axs[1].set_xlabel('Quarter')
        axs[1].set_ylabel('Total Revenue')
        axs[1].set_title(f'Quarterly Revenue for {year}')
        axs[1].set_xticks(df2['Quarter'])
        axs[1].set_xticklabels(list_of_quarters)
